  ...
  try {
    ...
    if (allProducts && allProducts.length > 0) {
      ...
      allProducts.slice(0, 5).forEach(product => {
        console.log(`  - ${product.name} (ID: ${product.id}, Category: ${product.category_id}, Status: ${product.status})`);
      });
...
export const testRelatedProductsQuery = async (categoryId = null, limit = 4) => {
