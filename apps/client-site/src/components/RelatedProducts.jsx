...
const RelatedProducts = memo(({
  ...
  const performFetch = async () => {
    ...
    try {
      ...
      switch (algorithm) {
        case "category":
          if (currentCategoryId) {
            ...
            if (currentProductId) {
              console.log(`❌ Excluding current product: ${currentProductId}`);
              query = query.neq("id", currentProductId);
            }
          } else {
            console.warn("⚠️ No currentCategoryId provided for category algorithm");
          }
          break;

        case "popular":
          console.log("🔥 Using popular algorithm (ordered by created_at desc)");
          // Simulate popular products - in real app, this would be based on sales/views
          query = query.order("created_at", { ascending: false });
          break;

        case "trending":
          console.log("📈 Using trending algorithm (ordered by stock_quantity desc)");
        ...
        case "personalized":
          ...
          try {
            ...
            console.log(`✅ Personalized returned:`, personalizedProducts?.length || 0, "products");
        ...
        case "recent":
          ...
          try {
            ...
            console.log(`📋 Recently viewed:`, recentlyViewed?.length || 0, "items");
            if (recentlyViewed.length > 0) {
              ...
              if (!recentError && recentProducts && recentProducts.length > 0) {
                ...
                console.warn("⚠️ No recent products found or error:", recentError);
              }
            }
        ...
        case "adaptive":
          ...
          try {
            ...
            console.log(`✅ Adaptive returned:`, adaptiveProducts?.length || 0, "products");
        ...
        default:
          console.log(`🔧 Using default algorithm (category-based) for: ${algorithm}`);
          // Default to category-based
          if (currentCategoryId) {
            console.log(`🏷️ Default: Filtering by category_id: ${currentCategoryId}`);
            query = query.eq("category_id", currentCategoryId);
            if (currentProductId) {
              console.log(`❌ Default: Excluding current product: ${currentProductId}`);
              query = query.neq("id", currentProductId);
            }
          } else {
            console.warn("⚠️ Default: No currentCategoryId, using popular fallback");
            query = query.order("created_at", { ascending: false });
          }
      }
      ...
      console.log("📊 Products summary:", products.map(p => ({ 
        id: p.id, 
        name: p.name, 
      ...
      if (
        ...
        console.log(`🔄 No products found for ${algorithm}, trying fallback...`);
        ...
        if (!fallbackError && fallbackData) {
          console.log(`✅ Fallback returned:`, fallbackData.length, "products");
